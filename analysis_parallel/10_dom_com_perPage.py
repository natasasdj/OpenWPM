...
df = df1[['site_id','link_id','respDom_domain2', 'respCompany']].drop_duplicates()
dom_pix1 = df['respDom_domain2'].value_counts()
dom_pix1_perc = dom_pix1/float(total_pages)*100
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir,'05a_respDomain_pix1_percPerPage.png'),format='png')
...
domcom['img_perc']=domcom['img_perc']/float(total_pages)*100
table_dir = os.path.join(main_dir,'tables')
fhand = open(os.path.join(table_dir,'baseDomain_Company_pix1_percPerPage_top30.txt'),'w+')
...
df = df1[['site_id','link_id','respCompany']].drop_duplicates()
com_pix1 = df['respCompany'].value_counts()
com_pix1_perc = com_pix1/float(total_pages)*100
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir, '05c_respCompany_pix1_percPerPage.png'),format='png')


### table top 30 companies
fhand = open(os.path.join(table_dir,'company_pix1_percPerPage_top30.txt'),'w+')
...
df = df0[['site_id','link_id','respDom_domain2', 'respCompany']].drop_duplicates()
dom_zero = df['respDom_domain2'].value_counts()
dom_zero_perc = dom_zero/float(total_pages)*100
...
plt.title('1-pixel Images')
plt.grid(True)
fig.savefig(os.path.join(fig_dir,'06a_respDomain_zero_percPerPage.png'),format='png')

### table domains - companies
domcom = df[['respDom_domain2','respCompany']].groupby(['respDom_domain2','respCompany']).size().reset_index(name='img_perc').sort_values('img_perc',ascending=False)
domcom['img_perc']=domcom['img_perc']/float(total_pages)*100
table_dir = os.path.join(main_dir,'tables')
fhand = open(os.path.join(table_dir,'baseDomain_Company_zero_percPerPage_top30.txt'),'w+')
...
df = df0[['site_id','link_id','respCompany']].drop_duplicates()
com_zero = df['respCompany'].value_counts()
com_zero_perc = com_zero/float(total_pages)*100
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir, '06c_respCompany_zero_percPerPage.png'),format='png')

### table top 30 companies
fhand = open(os.path.join(table_dir,'company_zero_percPerPage_top30.txt'),'w+')
