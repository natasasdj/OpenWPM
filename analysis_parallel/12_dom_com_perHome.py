...
df10=df1[df1['link_id']==0]

# response domains

df = df10[['site_id', 'respDom_domain2', 'respCompany']].drop_duplicates()
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir,'09a_respDomain_pix1_percPerHome.png'),format='png')
...
fhand = open(os.path.join(table_dir,'baseDomain_Company_pix1_percPerHome_top30.txt'),'w+')
...
df = df10[['site_id', 'respCompany']].drop_duplicates()
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir, '09c_respCompany_pix1_percPerHome.png'),format='png')


### table top 30 companies
fhand = open(os.path.join(table_dir,'company_pix1_percPerHome_top30.txt'),'w+')
...
df00 = df0[df0['link_id']==0]

df = df00[['site_id', 'respDom_domain2', 'respCompany']].drop_duplicates()
...
plt.title('1-pixel Images')
plt.grid(True)
fig.savefig(os.path.join(fig_dir,'10a_respDomain_zero_percPerHome.png'),format='png')

### table domains - companies
domcom = df[['respDom_domain2','respCompany']].groupby(['respDom_domain2','respCompany']).size().reset_index(name='img_perc').sort_values('img_perc',ascending=False)
domcom['img_perc']=domcom['img_perc']/float(total_domains)*100
table_dir = os.path.join(main_dir,'tables')
fhand = open(os.path.join(table_dir,'baseDomain_Company_zero_percPerHome_top30.txt'),'w+')
...
df = df00[['site_id', 'respCompany']].drop_duplicates()
...
plt.grid(True)
fig.savefig(os.path.join(fig_dir, '10c_respCompany_zero_percPerHome.png'),format='png')

### table top 30 companies
fhand = open(os.path.join(table_dir,'company_zero_percPerHome_top30.txt'),'w+')
