...
hash_dir = sys.argv[2]
#hash_dir = '/home/nsarafij/project/OpenWPM/file_hashing/db'
...
def checkFile(siteID, linkID, respID, filename,filedir):
    ...
    if hashDB.get(fhash) is None:
        ...
        # create symlink
        fname = hashDB.get(fhash.encode('utf8'))
        fpath = os.path.join(filedir,fname)
        # increment count in fileHashing.sqlite db
        fname.split("-")
        os.remove(filepath) 
        ...
        #print query
        cur1.execute(query)
    if ("html" in filename): os.remove(filepath)
...
for key, value in hashHtml:
    print "dbHTML", key, valuepd.isnull(df['resp_time_2']).sum()
